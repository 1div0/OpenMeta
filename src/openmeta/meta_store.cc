...
ByteArena& MetaStore::arena() noexcept
{
  return arena_;
}

const ByteArena& MetaStore::arena() const noexcept
{
  return arena_;
}

BlockId MetaStore::add_block(const BlockInfo& info)
{
  if (finalized_) {
...
EntryId MetaStore::add_entry(const Entry& entry)
{
  if (finalized_) {
...
void MetaStore::clear_indices() noexcept
{
...
std::span<const Entry> MetaStore::entries() const noexcept
{
  return std::span<const Entry>(entries_.data(), entries_.size());
}

const Entry& MetaStore::entry(EntryId id) const noexcept
{
  return entries_[id];
}

std::span<const EntryId> MetaStore::entries_in_block(BlockId block) const noexcept
{
  ...
  return std::span<const EntryId>(
    entries_by_block_.data() + span.start,
    span.count);
}

std::span<const EntryId> MetaStore::find_all(const MetaKeyView& key) const noexcept
{
  ...
  while (lo < hi) {
    ...
    if (cmp == 0) {
      return std::span<const EntryId>(
        entries_by_key_.data() + span.start,
        span.count);
    }
    if (cmp < 0) {
      hi = mid;
...
void MetaStore::rebuild_block_index()
{
  ...
  for (uint32_t i = 0; i < static_cast<uint32_t>(entries_by_block_.size()); ++i) {
...
void MetaStore::rebuild_key_index()
{
  ...
  for (uint32_t i = 1; i < static_cast<uint32_t>(entries_by_key_.size()); ++i) {
    const EntryId current = entries_by_key_[i];
    const int cmp = compare_key(arena_, entries_[run_repr].key, entries_[current].key);
    if (cmp != 0) {
      key_spans_.push_back(KeySpan{run_start, i - run_start, run_repr});
      run_start = i;
      run_repr = current;
    }
  }
