...
ByteArena& MetaEdit::arena() noexcept
{
  return arena_;
}

const ByteArena& MetaEdit::arena() const noexcept
{
  return arena_;
}

void MetaEdit::reserve_ops(size_t count)
{
  ops_.reserve(count);
}

void MetaEdit::add_entry(const Entry& entry)
{
  EditOp op;
...
void MetaEdit::set_value(EntryId target, const MetaValue& value)
{
  EditOp op;
...
void MetaEdit::tombstone(EntryId target)
{
  EditOp op;
...
std::span<const EditOp> MetaEdit::ops() const noexcept
{
  return std::span<const EditOp>(ops_.data(), ops_.size());
}

static MetaKey copy_key(const MetaKey& key,
  const ByteArena& src,
  ByteArena& dst)
{
  MetaKey out = key;
  ...
  case MetaKeyKind::XmpProperty:
    out.data.xmp_property.schema_ns =
      dst.append(src.span(key.data.xmp_property.schema_ns));
    out.data.xmp_property.property_path =
      dst.append(src.span(key.data.xmp_property.property_path));
    return out;
  ...
  case MetaKeyKind::PrintImField:
    out.data.printim_field.field =
      dst.append(src.span(key.data.printim_field.field));
    return out;
  case MetaKeyKind::JumbfField:
    out.data.jumbf_field.field =
      dst.append(src.span(key.data.jumbf_field.field));
    return out;
  case MetaKeyKind::JumbfCborKey:
    out.data.jumbf_cbor_key.key =
      dst.append(src.span(key.data.jumbf_cbor_key.key));
    return out;
  }
  return out;
}

static MetaValue copy_value(const MetaValue& value,
  const ByteArena& src,
  ByteArena& dst)
{
  MetaValue out = value;
  ...
  for (size_t e = 0; e < edits.size(); ++e) {
    ...
    for (size_t i = 0; i < ops.size(); ++i) {
      ...
      case EditOpKind::AddEntry: {
        ...
        entry.value = copy_value(entry.value, edit.arena(), out.arena());
      ...
      case EditOpKind::Tombstone: {
        ...
        out.entries_[op.target].flags |= (EntryFlags::Deleted | EntryFlags::Dirty);
        break;
      }
      }
    }
