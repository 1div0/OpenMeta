                    ...
                    ExifDecodeResult* status_out) noexcept
{
    ...
    const uint16_t u8_tags[]
        = { 0x0009, 0x000A, 0x0016, 0x001E, 0x0029, 0x002A };
    for (uint32_t i = 0; i < sizeof(u8_tags) / sizeof(u8_tags[0]); ++i) {
        const uint16_t t = u8_tags[i];
        if (!sony_read_u8(bytes, t, rounds, &u8v)) {
            continue;
                     ...
                     ExifDecodeResult* status_out) noexcept
{
    ...
    for (uint32_t ti = 0; ti < sizeof(vign_tags) / sizeof(vign_tags[0]); ++ti) {
                    ...
                    ExifDecodeResult* status_out) noexcept
{
    ...
    // VignettingCorrParams int16s[32] at 0x089d.
    {
        ...
        bool ok = true;
        for (uint32_t i = 0; i < v.size(); ++i) {
            ok = ok && sony_read_i16le(bytes, 0x089D + i * 2U, rounds, &v[i]);
        }
        if (ok) {
    ...
    // ChromaticAberrationCorrParams int16s[32] at 0x0945.
    {
        ...
        bool ok = true;
        for (uint32_t i = 0; i < v.size(); ++i) {
            ok = ok && sony_read_i16le(bytes, 0x0945 + i * 2U, rounds, &v[i]);
        }
        if (ok) {
                           ...
                           ExifDecodeResult* status_out) noexcept
{
    ...
    const bool is_slt_family = model.starts_with("SLT-")
                               || model.starts_with("ILCA-") || (model == "HV");
    ...
    for (uint32_t i = 0; i < cand_count; ++i) {
        ...
        if (tag == 0x2010) {
            ...
            if (is_lunar) {
                ...
                sony_decode_cipher_fields(raw, mk_prefix, "tag2010e", rounds,
                                          std::span<const SonyCipherField>(
                                              kSonyTag2010eFields,
                                              sizeof(kSonyTag2010eFields)
                                                  / sizeof(
                                                      kSonyTag2010eFields[0])),
                                          store, options, status_out);
                decode_sony_meterinfo_from_tag2010(raw, rounds, 0x04B8,
                                                   mk_prefix, store, options,
                                                   status_out);
            } else {
        ...
        if (tag == 0x9400) {
            ...
            if (is_lunar || is_slt_family || is_stellar) {
                sony_decode_cipher_fields(raw, mk_prefix, "tag9400a", rounds,
                                          std::span<const SonyCipherField>(
                                              kSonyTag9400aFields,
                                              sizeof(kSonyTag9400aFields)
                                                  / sizeof(
                                                      kSonyTag9400aFields[0])),
                                          store, options, status_out);
            } else {
        ...
        if (tag == 0x9406) {
            sony_decode_cipher_fields(raw, mk_prefix, "tag9406", 1,
                                      std::span<const SonyCipherField>(
                                          kSonyTag9406Fields,
                                          sizeof(kSonyTag9406Fields)
