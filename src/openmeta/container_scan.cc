...
static constexpr std::array<std::byte, 16> kJp2UuidExif = {
    std::byte{0x4a}, std::byte{0x70}, std::byte{0x67}, std::byte{0x54},
    std::byte{0x69}, std::byte{0x66}, std::byte{0x66}, std::byte{0x45},
    std::byte{0x78}, std::byte{0x69}, std::byte{0x66}, std::byte{0x2d},
    std::byte{0x3e}, std::byte{0x4a}, std::byte{0x50}, std::byte{0x32},
};

static constexpr std::array<std::byte, 16> kJp2UuidIptc = {
    std::byte{0x33}, std::byte{0xc7}, std::byte{0xa4}, std::byte{0xd2},
    std::byte{0xb8}, std::byte{0x1d}, std::byte{0x47}, std::byte{0x23},
    std::byte{0xa0}, std::byte{0xba}, std::byte{0xf1}, std::byte{0xa3},
    std::byte{0xe0}, std::byte{0x97}, std::byte{0xad}, std::byte{0x38},
};

static constexpr std::array<std::byte, 16> kJp2UuidXmp = {
    std::byte{0xbe}, std::byte{0x7a}, std::byte{0xcf}, std::byte{0xcb},
    std::byte{0x97}, std::byte{0xa9}, std::byte{0x42}, std::byte{0xe8},
    std::byte{0x9c}, std::byte{0x71}, std::byte{0x99}, std::byte{0x94},
    std::byte{0x91}, std::byte{0xe3}, std::byte{0xaf}, std::byte{0xac},
};
...
static void sink_emit(BlockSink* sink, const ContainerBlockRef& block) noexcept {
...
static bool read_u32be(std::span<const std::byte> bytes, uint64_t offset,
  ...
  const uint32_t v = (static_cast<uint32_t>(u8(bytes[offset + 0])) << 24) |
...
static bool read_u32le(std::span<const std::byte> bytes, uint64_t offset,
  ...
  const uint32_t v = (static_cast<uint32_t>(u8(bytes[offset + 0])) << 0) |
...
ScanResult scan_jpeg(std::span<const std::byte> bytes,
  ...
  while (offset + 2 <= bytes.size()) {
    ...
    const uint16_t marker =
        static_cast<uint16_t>(0xFF00U | static_cast<uint16_t>(marker_lo));
    ...
    if (marker == 0xFFE1) {
      if (seg_payload_size >= 6 && match(bytes, seg_payload_off, "Exif\0\0", 6)) {
        ...
        if (seg_payload_size >= 35 + 32 + 8) {
          ...
          if (read_u32be(bytes, full_off, &full_len) &&
            ...
            block.group =
                fnv1a_64(bytes.subspan(static_cast<size_t>(guid_off), 32)); // stable per GUID.
            sink_emit(&sink, block);
          }
        }
      }
      ...
      if (match(bytes, seg_payload_off, "ICC_PROFILE\0", 12)) {
        if (seg_payload_size >= 14) {
          ...
          block.id = marker;
          block.part_index = (seq > 0) ? static_cast<uint32_t>(seq - 1U) : 0U;
...
ScanResult scan_png(std::span<const std::byte> bytes,
                    std::span<ContainerBlockRef> out) noexcept {
  BlockSink sink;
  sink.out = out.data();
  sink.cap = static_cast<uint32_t>(out.size());
  ...
  while (offset + 12 <= bytes.size()) {
    ...
    uint32_t len = 0;
    uint32_t type = 0;
    if (!read_u32be(bytes, offset, &len) || !read_u32be(bytes, offset + 4, &type)) {
      sink.result.status = ScanStatus::Malformed;
      return sink.result;
    }
    ...
    if (type == fourcc('e', 'X', 'I', 'f')) {
      ...
      if (keyword_end + 3 <= end) {
        ...
        if (keyword_end - data_off == kXmpKeywordLen) {
          is_xmp = match(bytes, data_off, "XML:com.adobe.xmp", kXmpKeywordLen);
        }
...
ScanResult scan_webp(std::span<const std::byte> bytes,
                     std::span<ContainerBlockRef> out) noexcept {
  BlockSink sink;
  sink.out = out.data();
  sink.cap = static_cast<uint32_t>(out.size());
  ...
  while (offset + 8 <= file_end) {
    ...
    uint32_t type = 0;
    uint32_t size_le = 0;
    if (!read_u32be(bytes, offset, &type) || !read_u32le(bytes, offset + 4, &size_le)) {
      sink.result.status = ScanStatus::Malformed;
      return sink.result;
    }
...
ScanResult scan_gif(std::span<const std::byte> bytes,
                    std::span<ContainerBlockRef> out) noexcept {
  BlockSink sink;
  sink.out = out.data();
  sink.cap = static_cast<uint32_t>(out.size());
  ...
  while (offset < bytes.size()) {
    ...
    if (introducer == 0x21) { // extension
      ...
      if (label == 0xFF) { // application extension
        ...
        const bool is_xmp =
            match(bytes, app_id_off, "XMP Data", 8) &&
            match(bytes, app_id_off + 8, "XMP", 3);
        const bool is_icc =
            match(bytes, app_id_off, "ICCRGBG1", 8) &&
        ...
        if (is_xmp || is_icc) {
          ...
          block.kind = is_xmp ? ContainerBlockKind::Xmp : ContainerBlockKind::Icc;
...
static bool parse_bmff_box(std::span<const std::byte> bytes, uint64_t offset,
  ...
  if (offset + box_size > parent_end || offset + box_size > bytes.size()) {
    return false;
  }
...
static void scan_jp2_box_payload(std::span<const std::byte> bytes,
                                const BmffBox& box, BlockSink* sink) noexcept {
...
ScanResult scan_jp2(std::span<const std::byte> bytes,
                    std::span<ContainerBlockRef> out) noexcept {
  BlockSink sink;
  sink.out = out.data();
  sink.cap = static_cast<uint32_t>(out.size());
  ...
  if (!read_u32be(bytes, 0, &first_size) || !read_u32be(bytes, 4, &first_type)) {
    sink.result.status = ScanStatus::Malformed;
    return sink.result;
  }
...
ScanResult scan_jxl(std::span<const std::byte> bytes,
                    std::span<ContainerBlockRef> out) noexcept {
  BlockSink sink;
  sink.out = out.data();
  sink.cap = static_cast<uint32_t>(out.size());
...
static bool read_tiff_u16(const TiffConfig& cfg, std::span<const std::byte> bytes,
...
static bool read_tiff_u32(const TiffConfig& cfg, std::span<const std::byte> bytes,
...
static bool read_tiff_u64(const TiffConfig& cfg, std::span<const std::byte> bytes,
...
ScanResult scan_tiff(std::span<const std::byte> bytes,
                     std::span<ContainerBlockRef> out) noexcept {
  BlockSink sink;
  sink.out = out.data();
  sink.cap = static_cast<uint32_t>(out.size());
  ...
  while (stack_size > 0) {
    ...
    if (!cfg.bigtiff) {
      ...
      if (read_tiff_u32(cfg, bytes, next_ifd_off_pos, &next32)) {
        (void)push_offset(next32, std::span<uint64_t>(stack_buf), &stack_size);
      }
    } else {
      uint64_t n64 = 0;
      ...
      if (read_tiff_u64(cfg, bytes, next_ifd_off_pos, &next64)) {
        (void)push_offset(next64, std::span<uint64_t>(stack_buf), &stack_size);
      }
    }
      ...
      if (tag == 0x8769 || tag == 0x8825 || tag == 0xA005 || tag == 0x014A) {
        ...
        if (tag == 0x014A && count > 1) {
          ...
          for (uint64_t j = 0; j < count && j < 32; ++j) {
            ...
            if (!cfg.bigtiff) {
              uint32_t tmp = 0;
              if (read_tiff_u32(cfg, bytes, value_off + j * 4, &tmp)) {
                sub_off = tmp;
              }
            } else {
              if (type == 4 || type == 13) {
                ...
                (void)read_tiff_u64(cfg, bytes, value_off + j * 8, &sub_off);
              }
            }
            (void)push_offset(sub_off, std::span<uint64_t>(stack_buf), &stack_size);
          }
        } else {
          ...
          (void)push_offset(ptr, std::span<uint64_t>(stack_buf), &stack_size);
        }
      }
...
ScanResult scan_auto(std::span<const std::byte> bytes,
                     std::span<ContainerBlockRef> out) noexcept {
  ...
  if (bytes.size() >= 12 && match(bytes, 0, "RIFF", 4) && match(bytes, 8, "WEBP", 4)) {
    return scan_webp(bytes, out);
  }
  if (bytes.size() >= 6 && (match(bytes, 0, "GIF87a", 6) || match(bytes, 0, "GIF89a", 6))) {
  ...
  if (bytes.size() >= 4) {
    ...
    if ((b0 == 0x49 && b1 == 0x49) || (b0 == 0x4D && b1 == 0x4D)) {
      ...
      const uint16_t v = (b0 == 0x49)
                             ? static_cast<uint16_t>(u8(bytes[2]) | (u8(bytes[3]) << 8))
                             : static_cast<uint16_t>((u8(bytes[2]) << 8) | u8(bytes[3]));
      if (v == 42 || v == 43) {
  ...
  if (bytes.size() >= 12) {
    uint32_t size = 0;
    uint32_t type = 0;
    if (read_u32be(bytes, 0, &size) && read_u32be(bytes, 4, &type) && size == 12 &&
        match_bytes(bytes, 8, kJp2Signature.data(), 4)) {
      if (type == fourcc('j', 'P', ' ', ' ')) {
