...
static int compare_bytes(std::span<const std::byte> a,
...
MetaKey make_jumbf_cbor_key(ByteArena& arena, std::string_view key_text)
{
  MetaKey key;
...
int compare_key(const ByteArena& arena, const MetaKey& a, const MetaKey& b) noexcept
{
  ...
  case MetaKeyKind::ExifTag: {
    const int ifd_cmp =
      compare_bytes(arena.span(a.data.exif_tag.ifd), arena.span(b.data.exif_tag.ifd));
    if (ifd_cmp != 0) {
      return ifd_cmp;
    }
  ...
  case MetaKeyKind::IptcDataset: {
    if (a.data.iptc_dataset.record != b.data.iptc_dataset.record) {
      return (a.data.iptc_dataset.record < b.data.iptc_dataset.record) ? -1 : 1;
    }
    if (a.data.iptc_dataset.dataset != b.data.iptc_dataset.dataset) {
      return (a.data.iptc_dataset.dataset < b.data.iptc_dataset.dataset) ? -1 : 1;
    }
    return 0;
  }
  case MetaKeyKind::XmpProperty: {
    const int ns_cmp = compare_bytes(
      arena.span(a.data.xmp_property.schema_ns),
      arena.span(b.data.xmp_property.schema_ns));
    if (ns_cmp != 0) {
      return ns_cmp;
    }
    return compare_bytes(
      arena.span(a.data.xmp_property.property_path),
  ...
  case MetaKeyKind::PhotoshopIrb:
    if (a.data.photoshop_irb.resource_id < b.data.photoshop_irb.resource_id) {
      return -1;
    }
    if (a.data.photoshop_irb.resource_id > b.data.photoshop_irb.resource_id) {
      return 1;
  ...
  const MetaKey& b) noexcept
{
  ...
  case MetaKeyKind::ExifTag: {
    ...
    const int ifd_cmp = compare_bytes(a_ifd, arena.span(b.data.exif_tag.ifd));
    if (ifd_cmp != 0) {
      return ifd_cmp;
    }
    if (a.data.exif_tag.tag < b.data.exif_tag.tag) {
  ...
  case MetaKeyKind::XmpProperty: {
    const std::span<const std::byte> a_ns(
      reinterpret_cast<const std::byte*>(a.data.xmp_property.schema_ns.data()),
      a.data.xmp_property.schema_ns.size());
    const int ns_cmp = compare_bytes(a_ns, arena.span(b.data.xmp_property.schema_ns));
    if (ns_cmp != 0) {
      return ns_cmp;
    }
    const std::span<const std::byte> a_prop(
      reinterpret_cast<const std::byte*>(a.data.xmp_property.property_path.data()),
      a.data.xmp_property.property_path.size());
    return compare_bytes(a_prop, arena.span(b.data.xmp_property.property_path));
  }
  case MetaKeyKind::IccHeaderField:
    if (a.data.icc_header_field.offset < b.data.icc_header_field.offset) {
      return -1;
  ...
  case MetaKeyKind::PrintImField: {
    const std::span<const std::byte> a_field(
      reinterpret_cast<const std::byte*>(a.data.printim_field.field.data()),
