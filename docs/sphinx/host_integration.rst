...
``GPSAltitudeRef`` marked the height as below sea level. The camera-GPS concept
accepts XMP coordinates from the EXIF XMP schema; IPTC Extension
``LocationShown`` and ``LocationCreated`` coordinates remain decoded/queryable
but are not conflated with camera position. Use
