...
.. list-table::
   ...
   * - TIFF root and preview IFD tags
     - Use the known tag alias without a prefix, for example ``Make``,
       ``ModifyDate``, ``ImageWidth``.
   * - EXIF and interoperability IFD tags
     - Prefix with ``Exif:``, for example ``Exif:ExposureTime``,
       ``Exif:ISO``, ``Exif:CreateDate``.
   * - GPS IFD tags
     - Prefix with ``GPS:``, for example ``GPS:GPSLatitude``.
   * - MakerNote IFDs
     - Emit only when ``include_makernotes`` is true, using
       ``MakerNote:<ifd>:<tag-name-or-hex>``.
   * - XMP simple properties
     - Emit selected known namespaces as ``XMP:``, ``TIFF:``, ``Exif:``, or
       ``DC:`` plus the simple property name.
   ...
   * - EXR part 0 known aliases
     - Map selected standard attributes to common host names, for example
       ``owner -> Copyright``, ``capDate -> DateTime``,
