...
| RAF / X3F | Keep RAF header-declared FujiIFD/TIFF follow path, RAF header/directory geometry decode, RAFData geometry projection, standalone XMP fallback, and X3F header/PROP/section-JPEG reads stable with focused compare coverage | Only add read depth that directly improves downstream transfer/export confidence | Deepen remaining RAF model-specific tables and X3F image-processing sections if parity evidence shows real user-facing gaps |
...
| `RAF / X3F` | `Partial` | not a main writer lane | deeper RAF model-specific native tables and X3F image-processing sections beyond current header/PROP/JPEG-section lane |
