...
Current gaps for EXR-quality support:

- No dedicated `MetaKeyKind` for EXR attributes (currently no `ExrAttribute` key).
- No first-class storage for EXR `type_name`/built-in type code alongside value.
- No canonical representation rules yet for matrix/vector/box/channel-list types.
- No EXR container scanner/decode path in `container_scan` / `simple_meta_read`.
- No explicit lifecycle helpers for EXR-style constrained in-place header updates.
