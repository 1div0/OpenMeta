...
| Photoshop IRB (`PhotoshopIrb`) | Yes | Partial / Yes | Yes | Raw resources preserved, bounded interpreted subset, fixed-layout, Lightroom workflow, IPTC-NAA byte count, descriptor-header, simple descriptor-item, and enum/list/object header summaries, embedded IPTC/XMP/ICC decode |
| MPF | Yes | Yes | Yes | Basic TIFF-IFD decode |
| GeoTIFF (`GeotiffKey`) | Yes | Yes | Yes | GeoKeyDirectoryTag decode |
| BMFF derived fields (`BmffField`) | Yes | Yes | Yes | `ftyp`, item-info, `ipco`, `ipma`, `iref`, `grpl`, `iloc`/`idat`, graph summaries, aux semantics, primary item properties, primary metadata-carrier flags, primary sidecar summaries, and bounded primary-linked image roles |
...
- descriptor-header summaries, bounded simple descriptor item bodies, enum
  value summaries, and empty list/object descriptor headers for resources such
  as `LayerComps`,
...
| Descriptor-backed Photoshop resources | Descriptor header plus bounded item summaries | Descriptor version, remaining byte count, class ID/name, item count, complete simple item bodies for `bool`, `long`, `doub`, `UntF`, and `TEXT`, enum value summaries, and empty list/object descriptor headers are emitted; non-empty nested/list/object descriptor bodies remain out of scope for this subset and report truncation. |
...
- primary item properties, primary metadata-carrier flags, and primary sidecar
  counts/flags for linked metadata and image sidecars
...
- bounded primary-linked image-role fields and compact primary sidecar
  aggregate fields
