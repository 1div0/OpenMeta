...
- Current JPEG/TIFF prepare behavior:
  ...
  - C2PA: `Invalidate` on JPEG now emits a draft unsigned APP11 C2PA
    ...
    - `PreparedTransferPackagePlan` now provides the shared final-output
      package contract for current JPEG/TIFF rewrite paths:
      deterministic source ranges, prepared JPEG segments, and inline
      generated bytes can be written through one generic package writer.
