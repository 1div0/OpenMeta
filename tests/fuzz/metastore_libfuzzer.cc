...
[[noreturn]] static void fuzz_trap() noexcept
{
...
static uint32_t read_u32(std::span<const uint8_t> bytes, size_t offset) noexcept
{
...
static uint16_t read_u16(std::span<const uint8_t> bytes, size_t offset) noexcept
{
...
static MetaKey make_exif_key_with_ifd(ByteSpan ifd, uint16_t tag) noexcept
{
  MetaKey key;
...
static void verify_block_order(const MetaStore& store, BlockId block) noexcept
{
...
static void verify_lookup(const MetaStore& store,
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
  ...
  const uint32_t base_count =
    (size >= 1U) ? static_cast<uint32_t>(bytes[0] & 31U) : 0U;
    ...
    if (op == 0U) {
      if (!store.entries().empty()) {
        const EntryId target =
          static_cast<EntryId>(value % store.entries().size());
