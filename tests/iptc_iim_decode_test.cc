...
TEST(IptcIimDecodeTest, DecodesDatasetsAndPreservesDuplicates)
{
    ...
    MetaStore store;
    const IptcIimDecodeResult r = decode_iptc_iim(
        std::span<const std::byte>(iptc.data(), iptc.size()), store,
        EntryFlags::None);
    ...
    {
        const std::span<const std::byte> b = store.arena().span(e0.value.data.span);
        ASSERT_EQ(b.size(), 5U);
    ...
    {
        const std::span<const std::byte> b = store.arena().span(e1.value.data.span);
        ASSERT_EQ(b.size(), 3U);
...
TEST(IptcIimDecodeTest, EnforcesMaxDatasetsLimit)
{
    ...
    MetaStore store;
    const IptcIimDecodeResult r = decode_iptc_iim(bytes, store, EntryFlags::None, options);
