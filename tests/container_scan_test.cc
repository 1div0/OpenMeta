...
TEST(ContainerScan, JpegSegments) {
  ...
  const std::array<std::byte, 14> exif_payload = {
      ...
      std::byte{0x00}, std::byte{0x00}, std::byte{'I'}, std::byte{'I'},
      std::byte{0x2A}, std::byte{0x00}, std::byte{0x08}, std::byte{0x00},
...
TEST(ContainerScan, PngChunks) {
  ...
  const std::array<std::byte, 8> exif = {std::byte{'I'}, std::byte{'I'},
...
TEST(ContainerScan, Jp2AndJxlBoxes) {
  ...
  uuid_payload.insert(uuid_payload.end(), xmp_uuid.begin(), xmp_uuid.end());
