...
static MetaKey make_exif_key(ByteSpan ifd, uint16_t tag) noexcept
{
  MetaKey key;
...
static void verify_block_order(const MetaStore& store, BlockId block)
{
...
static void verify_lookup_tags(const MetaStore& store,
...
static void meta_store_op_stream(const std::vector<Op>& ops)
{
  MetaStore store;
  ...
  for (size_t i = base_count; i < ops.size(); ++i) {
    ...
    const EntryId target =
      static_cast<EntryId>(ops[i].target % base_entries.size());
...
FUZZ_TEST(MetaStoreFuzz, meta_store_op_stream)
  .WithDomains(fuzztest::VectorOf(fuzztest::StructOf<Op>(
    fuzztest::InRange<uint8_t>(0, std::numeric_limits<uint8_t>::max()),
    fuzztest::InRange<uint16_t>(0, std::numeric_limits<uint16_t>::max()),
    fuzztest::Arbitrary<uint32_t>(),
    fuzztest::Arbitrary<uint32_t>())).WithMaxSize(64));
